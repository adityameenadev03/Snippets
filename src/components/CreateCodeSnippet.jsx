...
import { addSnippet, deleteSnippet } from "../reducers/snippetSlice";
...
const CreateCodeSnippet = () => {
    ...
    const user = useSelector(store => store.user.user)
     ...
     const [codeSnippet, setCodeSnippet] = useState({
        ...
        tags: [],
        tabs: [
            { id: 1, title: 'Snippet 1', content: '' },
            { id: 2, title: 'Snippet 2', content: '' },
    ...
    const updateDatabase = async (collectionName, doc) => {
        try {
            const docRef = await addDoc(collection(db, collectionName), { ...doc, userId: user.uid + "dad" });
            console.log("saved", docRef)
        } catch (e) {
            console.error("Error adding document: ", e);
            dispatch(deleteSnippet(codeSnippet))
        }
    }
    ...
    let snipp = [
        ...
        },
        {
            ...
            "createdAt": 1676916992880,
            "isDeleted": true,
            "isFavorites": false,
            "folderId": "",
            "updatedAt": 1676917001164,
            "tabs": [
                {
                    "id": 0,
                    "title": "Fragment 1",
    ...
    async function uploadDocuments(data) {
        try {
            for (const document of data) {
                ...
                await addDoc(collection(db, "snippets"), { ...document, userId: "OuGK41jW9sO7iGSLS1u2bEF8eC23" });
            }
    ...
    const handleSubmit = async (event) => {
        ...
        // const result = dispatch(addSnippet(codeSnippet))

        const done = await updateDatabase('snippets', snipp[count])
        ...
        if (done) {
            ...
            setShowImage(false)
            setActiveTab(codeSnippet.tabs[0].id)
        }
            ...
            <form action=""
                ...
                <div className="note_container">
                    ...
                    {codeSnippet?.tabs.map(tab => {
                        if (tab.id === activeTab) {
                            return (
                                <div key={tab.id} className='active_tab_div'>
